...
function get_ajax_comment(aid, start, limit,orderWay,type) {
    var url = 'http://www.lzqcode.com/plus'+"/comment_ajax.php";
...
jDedemao(document).on('click', '.commentVote', function (event) {
    ...
    jDedemao.post('http://www.lzqcode.com/plus'+'/comment_ajax.php',{action:ftype,fid:fid},function (data) {
        if (data.code==0){
...
function getQQInfo(obj) {
    ...
    if(type=='QQ号' && qq.length>5){
        jDedemao.getJSON('http://www.lzqcode.com/plus'+'/comment_ajax.php',{action:'getQQInfo',qq:qq},function (data) {
            if (data.code==0){
